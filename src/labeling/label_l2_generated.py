...
Usage (from repo root):
  python src/labeling/label_l2_generated.py --overwrite --n-batch 512
  ...
  python src/labeling/label_l2_generated.py \\
    ...
    --output-dir outputs --n-batch 512
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=N_BATCH,
        help=f"llama.cpp n_batch (default {N_BATCH}; OOM → try 256)",
    )
    ...
    parser.add_argument(
        "--no-tensor-split",
        action="store_true",
        help="Force no tensor_split (already the default on this branch)",
    )
