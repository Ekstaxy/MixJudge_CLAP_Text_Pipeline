...
改用 llama-cpp-python 載入 Q4_K_M GGUF;單卡全層上 GPU,多卡 (如 Kaggle T4x2) 自動 tensor_split。
...
Kaggle T4x2 (自動偵測 2 GPU → tensor_split 0.5,0.5):
  python src/labeling/labeling_gguf.py --model-path /kaggle/input/.../xxx.gguf
  python src/labeling/labeling_gguf.py --tensor-split 0.5,0.5 --n-batch 256
...
# n_batch     : 只影響速度,不影響標註品質。T4x2 若 OOM 可降到 256
# n_gpu_layers: -1=全上 GPU。降到 CPU 會更慢,品質幾乎無好處
# tensor_split: 多卡比例 (Kaggle T4x2 → 0.5,0.5)。31B Q4 單卡 16GB 裝不下
...
def resolve_tensor_split(
    ...
    *,
    ...
    """Explicit split, else equal split across all visible GPUs when count >= 2."""
    if disable:
        return None
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=N_BATCH,
        help=f"llama.cpp n_batch / n_ubatch (預設 {N_BATCH}; T4x2 OOM 可降到 256)",
    )
    parser.add_argument(
        "--n-gpu-layers",
        type=int,
    ...
    parser.add_argument(
        ...
        default=None,
        help="多卡比例,例如 0.5,0.5 (Kaggle T4x2)。預設: >=2 GPU 時自動均分",
    )
    parser.add_argument(
        "--no-tensor-split",
        action="store_true",
        help="強制單卡 (關閉自動 multi-GPU split)",
    )
